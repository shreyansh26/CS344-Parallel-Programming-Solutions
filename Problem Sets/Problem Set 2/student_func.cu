                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for(int yy=0; yy<filterWidth; yy++) {
    for(int xx=0; xx<filterWidth; xx++) {
      ...
      int im_pos = imY * numCols + imX;
      int filter_pos = filterY * filterWidth + filterX;
      temp += (filter[filter_pos] * (float)(inputChannel[im_pos]));
    }
  }
  outputChannel[idx] = (unsigned char) temp;
}
                      ...
                      unsigned char* const blueChannel)
{
  ...
  redChannel[idx] = (unsigned char)inputImageRGBA[idx].x;
  greenChannel[idx] = (unsigned char)inputImageRGBA[idx].y;
  blueChannel[idx] = (unsigned char)inputImageRGBA[idx].z;
}
